...
| `upload <xlsx> [--name <标题>]` | 把非契约的独立 xlsx 上传为同一文件夹里的查看用表格，不做验证；默认标题为文件名去掉扩展名，同名已存在则覆盖内容而非重复新建。货柜成本结果不用它交付，而是写入原表格的「成本计算结果」工作表 |
| `url <ref>` | 打印链接 |
...
- 合计、分单位合计和 config 公式以 `DATA_END` 定位明细范围，在 `DATA_END` 前插入行不需要改公式。
